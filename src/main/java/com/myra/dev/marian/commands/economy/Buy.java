    ...
    public void execute(CommandContext ctx) throws Exception {
        ...
        // Buy role
            ...
            if (ctx.getMember().getRoles().contains(ctx.getGuild().getRoleById(roleInfo.getId()))) {
                ...
                if (balance + roleInfo.getPrice() / 2 > Config.ECONOMY_MAX) {
                    ...
                    EmbedBuilder removeRole = new EmbedBuilder()
                            ...
                            .setColor(Utilities.getUtils().blue)
                            .setDescription("You already own this role. You can't get the full amount of money, because you would be too rich. Do you want to sell this role for `" + sellingPrice + "` " + new Database(ctx.getGuild()).getNested("economy").getString("currency"));
                ...
                // Maximum amount of money wouldn't be reached
                    ...
                    EmbedBuilder removeRole = new EmbedBuilder()
                            .setAuthor("buy", null, ctx.getAuthor().getEffectiveAvatarUrl())
                            .setColor(Utilities.getUtils().blue)
                            .setDescription("You already own this role. Do you want to sell this role for " + roleInfo.getPrice() / 2 + " " + new Database(ctx.getGuild()).getNested("economy").getString("currency"));
                        ...
                        Myra.WAITER.waitForEvent(
                                ...
                                e -> { // On event
                                    ...
                                    if (response.equalsIgnoreCase("yes") || response.equalsIgnoreCase("ye")) {
                                        final String currency = new Database(ctx.getGuild()).getNested("economy").getString("currency"); // Get currency
                                        new Success(ctx.getEvent())
                                                .setCommand("buy")
                                                .setEmoji("\uD83D\uDED2")
                                                .setMessage(String.format("Sold **%s** for `%s`%s", role.getAsMention(), roleInfo.getPrice(), currency))
                                                .send();
